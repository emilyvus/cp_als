...
def compute_ttest(mdf, population_name_1 = "GBR", population_name_2 = "ASW"):
...
def mean_variant_count_one_population(df,genome_ids):
    ...
    1. For each genome, count number of variants per gene
    2. For each gene, count the total number of variants across all genome
    3. Take this total, divide by the number of genomes
...
def create_mean_variant_count_all_population_df(root_dir, genomes: Dict, outputfile: str=None):
    """
    This method works on all population. 

    It outputs a data frame where each column is a population. The data frame is used for statistical analysis like t-test, anova, tukey.
...
def create_total_variant_count_all_population_df(root_dir, genomes: Dict, outputfile: str=None):
    ...
    It outputs a data frame, each column is for one population and includes the total variant count per gene, across all genomes.

    The output data frame is used by statistical analysis like t-test, anova, tukey.
...
def create_per_genome_variant_mean_all_population_df(root_dir, genomes: Dict, outputfile: str=None):
    ...
    For each population, it creates a list of numbers, each number is the mean of variant counts one genome has across all 34 genes.

    For example, the list of KHV population would include 99 numbers since there are 99 genomes. One number (representing a genome) would be the mean amount of variant counts across all genes for that genome.
    ...
    # only to save to the file for checking later.
    if outputfile:
