...
def do_one_gene(df,cdf, outfile_linked_clinvar,outfile_linked_clinvar_noindel, outfile_parsed_INFO, outfile_per_genome_counts):
    ...
    # work on the per gene, per position (each position is a row)
    ...
    df['genomes'] = df.apply(lambda row: find_matching_columns(row,["0|1", "1|0", "1|1"]),axis=1)
    ...
    # now, work on the per gene, per genome id, each genome is a row
    gdf =df[vn_genome_ids] # filter all the genome ids, each genome is a column
    tdf = gdf.transpose()
